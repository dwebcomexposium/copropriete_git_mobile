...
;jQuery(document).ready(function($){
	...
	if ( $('.exhibitor-search-form').length === 1 ) {
		...
		$('.esf-as-heading').find('.esf-js-placeholder').each(function(){
			$(this).after('<button type="button" class="esf-remove-filters hidden" aria-hidden="true">'+$(this).attr('title')+'<i class="icon-close" aria-hidden="true"></i></button>').remove();
		});
		...
		*/
		...
		$panel_content.on('click', '.esf-has-sub-items > .esf-choice-container .esf-next-lvl', function() {
			...
			if ( $next_lvl.find('li:first').filter('.esf-sublvl-heading').length === 0 ) {
				$next_lvl.find('li:first').before('<li class="esf-sublvl-heading"><button type="button" class="esf-back-to-prev-lvl"><i class="icon-arrow-left" aria-hidden="true"></i>' + label_text + '</button> <label for="'+uniqid+'_select_all"><span class="esf_select_all_label">'+select_all_txt+'</span> <input type="checkbox" id="'+uniqid+'_select_all" class="esf-checkbox-select-all"'+checked_state+' /></label></li>');
			}
